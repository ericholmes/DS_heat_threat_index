...
load("data/DS_HTI_Wtemp_RVB.Rdata")

listemp <- RVB[ is.na(RVB$Param_val) == F,]
...
lis$heat <- ifelse(lis$mintemp > 22 | lis$maxtemp > 24, "over", "under")
...
heat_timeline <- ggplot() + 
  ...
  scale_y_continuous(breaks = seq(2000, 2024, 1))
...
ggplot(lis[lis$Year == c(2015:2024),], aes(x = jday, y = meantemp)) + theme_bw() +
  geom_line(data = listemp[listemp$Year == c(2015:2024),], aes(x = jdayfrac, y = Param_val)) +
  geom_ribbon(data = listemp2[listemp2$Year == c(2015:2024),], alpha = .5, 
              aes(ymin = 18, ymax = meantemp, group = heat_ID, fill = Key)) + scale_fill_brewer(palette = "Set1") +
  theme(legend.position = "bottom")  + labs(x = NULL) + ylim(18,26)+
  geom_hline(yintercept = c(22,24), linetype = 2) +
  scale_x_continuous(limits = c(150, 270),
